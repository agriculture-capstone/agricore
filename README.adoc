...
. Using the user you just created, create a database and grant the proper permissions to the user. The recommended default name for the database is `agricoredev`.

	psql -U boresha
	CREATE DATABASE agricoredev;
	GRANT ALL PRIVILEGES ON DATABASE agricoredev TO boresha;
...
. Outside of the postgres shell, run the initialization commands for initializing the database.
	...
	psql -f db-init.sql agricoredev boresha
