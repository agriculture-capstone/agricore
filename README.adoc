...
To set up a database for the Agricore, follow the steps below.

. The Agricore uses PostgreSQL 10.1, although any version >=9.1 should work as well.
. Create database with correct user permissions, the recommended default names are `agricore-dev` for the database name, and `boresha` for the user.
. Run the `psql` shell using psql -d <databasename> -U <username>.
. Install the `uuid-ossp` extension, `CREATE EXTENSION "uuid-ossp"`, this is already packaged but not loaded for postgres >=9.1.
. Run the initialization commands for creating the database tables, `\i db-init.sql`, the shell must be started in the same place as `db-init.sql` for this step.
